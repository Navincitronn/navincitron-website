(() => {
    ...
    function restorePendingGeniusDocumentWrite() {
        if (typeof restoreGeniusDocumentWrite === "function") {
            const restore = restoreGeniusDocumentWrite;
            restoreGeniusDocumentWrite = null;
            restore();
        }
    }

    function removePendingGeniusEmbedScript() {
        if (activeGeniusEmbedScript && activeGeniusEmbedScript.isConnected) {
            activeGeniusEmbedScript.remove();
        }
        activeGeniusEmbedScript = null;
...
<body>
    ...
    function appendGeniusDocumentWrite(host, markup, renderToken) {
        if (renderToken !== geniusEmbedRenderToken || !host || !host.isConnected) return;

        const html = String(markup || "");
        if (!html) return;

        // Genius historically emits its actual embed iframe with document.write().
        // Intercept that write and place the emitted markup directly beside the
        // official rg_embed_link node instead of wrapping it in another srcdoc iframe.
        const template = document.createElement("template");
        template.innerHTML = html;

        for (const oldScript of Array.from(template.content.querySelectorAll("script"))) {
    ...
    function findDirectGeniusFrame(host, geniusSong, songId) {
        if (!host || !host.isConnected || Number(songId) !== lastGeniusSongId) return null;
        const frame = host.querySelector("iframe");
        if (!frame) return null;

        activeEmbedFrame = frame;
        frame.classList.add("lyrics-genius-frame");
    ...
    function renderGeniusOfficialEmbed(geniusSong) {
        ...
        if (
            ...
            embedContainer.querySelector(`.rg_embed_link[data-song-id="${songId}"]`)
        ) {
            return;
        }
        ...
        host.className = "lyrics-genius-direct-host";
        ...
        const artist = geniusSong.artist ? ` by ${geniusSong.artist}` : "";
        anchor.textContent = `Read “${geniusSong.title || "this song"}”${artist} on Genius`;
        embedLink.appendChild(anchor);
        host.appendChild(embedLink);
        embedContainer.appendChild(host);

        const observer = new MutationObserver(() => {
            if (renderToken !== geniusEmbedRenderToken) {
                observer.disconnect();
                return;
            }
            if (findDirectGeniusFrame(host, geniusSong, songId)) {
                observer.disconnect();
            }
        });
        observer.observe(host, { childList: true, subtree: true });

        // Mount Genius exactly as its current official embed snippet specifies:
        // rg_embed_link in the host document + /songs/{id}/embed.js. The previous
        // implementation ran that snippet inside an additional srcdoc iframe.
        // That extra browsing-context layer is no longer inserted here.
        const script = document.createElement("script");
        script.crossOrigin = "anonymous";
        script.src = geniusSong.embedScriptUrl || `https://genius.com/songs/${songId}/embed.js`;
        script.async = true;
        activeGeniusEmbedScript = script;

        const originalWrite = document.write;
        const originalWriteln = document.writeln;
        const writeProxy = (...parts) => appendGeniusDocumentWrite(host, parts.join(""), renderToken);
        const writelnProxy = (...parts) => appendGeniusDocumentWrite(host, `${parts.join("")}\n`, renderToken);

        document.write = writeProxy;
        document.writeln = writelnProxy;
        restoreGeniusDocumentWrite = () => {
        ...
        script.addEventListener("load", () => {
            if (activeGeniusEmbedScript === script) activeGeniusEmbedScript = null;
            restorePendingGeniusDocumentWrite();
            findDirectGeniusFrame(host, geniusSong, songId);
            window.setTimeout(() => findDirectGeniusFrame(host, geniusSong, songId), 250);
            window.setTimeout(() => findDirectGeniusFrame(host, geniusSong, songId), 1000);
        }, { once: true });
        ...
        // Safety restore in case a browser/provider edge case never fires load/error.
        window.setTimeout(() => {
